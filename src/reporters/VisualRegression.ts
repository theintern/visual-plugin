...
import saveDifferenceImage from './util/saveDifferenceImage';
...
export default class VisualRegression {
	...
	constructor(executor: Executor, options: Options) {
	...
	private writeTest(test: VisualRegressionTest) {
	...
	private writeSuite(suite: Suite) {
		if (suite.parent) {
			return Promise.resolve();
		}
		...
		return save(
			...
			const source = join(__dirname, 'util', 'assets', 'main.css');
