...
export interface Options {
	pixelSkip?: number;
	pixelTolerance?: number | RGBAColorArray;
}

export default class PngJsImageComparator implements ImageComparator {
	pixelSkip: number;

	pixelTolerance: RGBAColorArray;

	constructor(options: Options = {}) {
		this.pixelTolerance = getRGBA(
			options.pixelTolerance || config!.comparator!.pixelTolerance!
		)!;
		...
		return Promise.all([
			...
			([
				...
				if (
					...
					throw new Error(
						'PNGs are different sizes. Expected (' +
							baselinePng.width +
							'x' +
							baselinePng.height +
							'); ' +
	...
	private _loadImage(
		...
		const loadPromise =
			typeof image === 'string'
				? this._loadPng(image)
				: Promise.resolve(PNG.sync.read(image));

		return loadPromise.then(png => {
			return {
				png,
				metadata: {
					height: png.height,
